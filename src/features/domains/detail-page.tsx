...
import { StrategyPanel } from "@/features/founder-os/strategy-panel";
...
export function DetailPage({ domain, id }: { domain: string; id: string }) {
  ...
  return <div className="detail-page">
    ...
    <EntityIntelligence type="project" id={id}/><DependencyPanel type="project" id={id}/><StrategyPanel type="project" id={id}/>
...
function GenericDetail({ domain, record }: { domain: string; record: Item }) {
  ...
  return <div className="detail-page"><Link className="back-link" href={`/${domain}`}>← Back to {domain}</Link><header className="detail-header"><div><p className="eyebrow">{domain}</p><h1>{title}</h1><p>{String(record.description ?? record.content ?? record.notes ?? "Saved in your private workspace.")}</p></div></header>{domain === "leads" || domain === "opportunities" ? <BusinessCommunication entity={domain === "leads" ? "lead" : "opportunity"} id={record.id}/> : null}{["tasks", "projects"].includes(domain) && <><DependencyPanel type={domain === "tasks" ? "task" : "project"} id={record.id}/><StrategyPanel type={domain === "tasks" ? "task" : "project"} id={record.id}/></>}{entity?<AttachmentSection entityType={entity} entityId={record.id}/>:null}{driveType?<DriveFiles entityType={driveType} entityId={record.id}/>:null}</div>;
}
