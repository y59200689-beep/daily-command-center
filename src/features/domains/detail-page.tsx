...
export function DetailPage({ domain, id }: { domain: string; id: string }) {
  ...
  return <div className="detail-page">
    ...
    <div className="detail-tabs">{["tasks", "notes", "milestones", "decisions", "content", "invoices", "expenses", "files", "activity"].map((name) => <button className={tab === name ? "active" : ""} onClick={() => setTab(name)} key={name}>{name[0].toUpperCase() + name.slice(1)} <span>{related[name]?.length ?? 0}</span></button>)}</div>
    <div className="detail-grid"><section><p className="eyebrow">Linked {tab}</p><h2>{items.length ? `${items.length} connected item${items.length === 1 ? "" : "s"}.` : `No linked ${tab} yet.`}</h2>{items.map((item) => <RelatedItem item={item} tab={tab} key={item.id}/>)}</section>
...
function GenericDetail({ domain, record }: { domain: string; record: Item }) {
  ...
  return <div className="detail-page"><Link className="back-link" href={`/${domain}`}>← Back to {domain}</Link><header className="detail-header"><div><p className="eyebrow">{domain}</p><h1>{title}</h1><p>{String(record.description ?? record.content ?? record.notes ?? "Saved in your private workspace.")}</p></div></header></div>;
}
