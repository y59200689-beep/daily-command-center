...
export function FounderDashboard() {
  ...
  return <div className="domain-page founder-page">
    <header className="task-context-header"><div><nav className="task-context-header__breadcrumb" aria-label="Breadcrumb"><span>Business</span><span>/</span><span className="current">Founder</span></nav><div className="task-context-header__title-row"><h1>{data.company.name}</h1><span className="task-context-header__total-badge">Operating system</span></div><p className="task-context-header__description">Company health, signal, and the next action that protects momentum.</p></div>{data.nextAction ? <Link className="founder-next-action" href={data.nextAction.route}><p className="eyebrow">Next founder action</p><strong>{data.nextAction.title}</strong><span>{data.nextAction.reason}</span></Link> : <div className="founder-next-action"><p className="eyebrow">Next founder action</p><strong>No urgent signal</strong><span>Connect a source or record an operational signal to begin.</span></div>}</header>
...
function Setup({ projects, projectId, setProjectId, busy, setup }: { projects: Array<{ id: string; name: string }>; projectId: string; setProjectId: (value: string) => void; busy: boolean; setup: () => Promise<void> }) { return <div className="domain-page founder-page"><header className="task-context-header"><div><nav className="task-context-header__breadcrumb" aria-label="Breadcrumb"><span>Founder</span><span>/</span><span className="current">Setup</span></nav><div className="task-context-header__title-row"><h1>Para Officinal.</h1></div><p className="task-context-header__description">Connect the existing Para Officinal project to start monitoring company operations.</p></div></header><section className="data-surface founder-setup"><p className="eyebrow">Company model</p><label htmlFor="founder-project">Primary project</label><select id="founder-project" value={projectId} onChange={(event) => setProjectId(event.target.value)}><option value="">Choose later</option>{projects.map((project) => <option value={project.id} key={project.id}>{project.name}</option>)}</select><Button disabled={busy} onClick={() => void setup()}>{busy ? "Setting up…" : "Set up Para Officinal"}</Button></section></div>; }
