...
function SourceEdit({ item, onClose, onSave }: { item: Row; onClose: () => void; onSave: (values: Row) => Promise<void> }) {
  ...
  return <Modal open onClose={onClose} title="Edit source"><form noValidate className="simple-form" onSubmit={(event) => { event.preventDefault(); const output: Row = Object.fromEntries(Object.entries(values).map(([key, value]) => [key, value || null])); if (typeof output.accessed_at === "string") output.accessed_at = new Date(output.accessed_at).toISOString(); void onSave(output); }}>{Object.entries(values).map(([key, value]) => key === "notes" ? <label key={key}>Notes<textarea className="resize-none" rows={4} value={value} onChange={(event) => set(key, event.target.value)} /></label> : <label key={key}>{label(key)}<input type={key === "accessed_at" ? "datetime-local" : key === "published_at" || key === "freshness_expires_at" ? "date" : "text"} value={value} onChange={(event) => set(key, event.target.value)} /></label>)}<div className="modal__actions"><Button emphasis="ghost" onClick={onClose}>Cancel</Button><Button intent="brand" type="submit">Save changes</Button></div></form></Modal>;
}
