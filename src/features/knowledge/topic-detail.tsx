...
export function TopicDetail({ id }: { id: string }) {
  ...
  const load = useCallback(async () => {
    ...
    if (body.schemaStatus === "unavailable") { setError("This detail is unavailable until the V9 knowledge schema is installed."); return; }
    if (response.ok) {
      setData(body);
      setError("");
  ...
  return <main className="domain-page knowledge-topic">
    <header className="task-context-header"><div><nav className="task-context-header__breadcrumb" aria-label="Breadcrumb"><span>Intelligence</span><span>/</span><Link href="/knowledge" style={{ color: "var(--muted)", textDecoration: "none" }}>Knowledge</Link><span>/</span><span className="current">{words(topic.domain)}</span></nav><div className="task-context-header__title-row"><h1>{String(topic.title)}</h1><span className="task-context-header__total-badge">{words(topic.health)}</span></div><p className="task-context-header__description">{words(topic.health)} · {words(topic.status)}</p></div><div className="task-context-header__actions"><Button emphasis="outline" onClick={() => void patchTopic(topic.status === "paused" ? "active" : "paused")}>{topic.status === "paused" ? "Resume" : "Pause"}</Button><Button onClick={() => void patchTopic("complete")}>Complete</Button></div></header>
