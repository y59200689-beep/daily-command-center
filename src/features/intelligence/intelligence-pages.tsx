...
type WeeklyReview = {
  financial?: Awaited<ReturnType<typeof import('@/lib/financial-server').loadFinancialControl>> | null;
  periodStart: string;
  periodEnd: string;
  ...
  growth?: { activeExperiments: number; dealsWon: number; dealsLost: number; dealReviews: number };
  operations?: { runsCompleted: number; runsFailed: number; blockedRuns: number; runsOverdue: number; qualityIncidents: number; sopsNeedingReview: number; recurringOperationsDue: number; improvementsAdopted: number; processHealthConcerns: number };
  team?: { openDelegations: number; completedDelegations: number; overdueDelegations: number; blockedDelegations: number; waitingOnTeam: number; waitingOnMe: number; pendingHandoffs: number; ownershipGaps: number; backupGaps: number; activePeople: number };
};
export function WeeklyReviewPage() {
  ...
  return (
    <div className="intelligence-page">
      ...
      {review.financial?<section className="data-surface"><p className="eyebrow">Financial Control</p><p>Current cash and forward outlook — not historical weekly balances.</p>{review.financial.cash.map(a=><p key={a.id}>{a.name}: {a.balance===null?'Unavailable':a.currency+' '+a.balance.toLocaleString()} · {a.stale?'Needs refresh':'Recorded balance'}</p>)}{review.financial.forecasts.map(f=><p key={f.currency}>{f.currency}: next 30 days expected in {f.buckets[0].inflows.toLocaleString()}, out {f.buckets[0].outflows.toLocaleString()}</p>)}{review.financial.buffer.map(b=><p key={b.currency}>{b.currency} buffer: {b.state}</p>)}{review.financial.risks.slice(0,3).map(r=><p key={r.key}><Link href={r.route}>{r.title}: {r.evidence}</Link></p>)}<Link href="/financial-control">Open Financial Control</Link></section>:null}
      ...
      {review.growth ? <section className="data-surface weekly-business-review"><div className="section-heading section-heading--small"><div><p className="eyebrow">Sales & Growth</p><h2>Pipeline & Experiments</h2></div><Link href="/growth">Open growth</Link></div><div className="metric-ledger metric-ledger--four"><Metric value={String(review.growth.activeExperiments)} label="Active experiments"/><Metric value={String(review.growth.dealsWon)} label="Deals won"/><Metric value={String(review.growth.dealsLost)} label="Deals lost"/><Metric value={String(review.growth.dealReviews)} label="Deal reviews recorded"/></div></section> : null}
      {review.operations ? <section className="data-surface weekly-business-review"><div className="section-heading section-heading--small"><div><p className="eyebrow">Operations</p><h2>Runs & Reliability</h2></div><Link href="/operations">Open operations</Link></div><div className="metric-ledger metric-ledger--four"><Metric value={String(review.operations.runsCompleted)} label="Runs completed"/><Metric value={String(review.operations.runsOverdue)} label="Runs overdue"/><Metric value={String(review.operations.runsFailed)} label="Runs failed"/><Metric value={String(review.operations.blockedRuns)} label="Blocked runs"/><Metric value={String(review.operations.qualityIncidents)} label="Quality incidents"/><Metric value={String(review.operations.sopsNeedingReview)} label="SOPs needing review"/><Metric value={String(review.operations.recurringOperationsDue)} label="Recurring operations due"/><Metric value={String(review.operations.improvementsAdopted)} label="Improvements adopted"/></div></section> : null}
      {review.team ? <section className="data-surface weekly-business-review"><div className="section-heading section-heading--small"><div><p className="eyebrow">Team & Delegation</p><h2>Coordination & Responsibility</h2></div><Link href="/team">Open team</Link></div><div className="metric-ledger metric-ledger--four"><Metric value={String(review.team.openDelegations)} label="Open delegations"/><Metric value={String(review.team.completedDelegations)} label="Completed delegations"/><Metric value={String(review.team.overdueDelegations)} label="Overdue delegations"/><Metric value={String(review.team.blockedDelegations)} label="Blocked delegations"/><Metric value={String(review.team.waitingOnTeam)} label="Waiting on team"/><Metric value={String(review.team.waitingOnMe)} label="Waiting on me"/><Metric value={String(review.team.ownershipGaps)} label="Ownership gaps"/><Metric value={String(review.team.backupGaps)} label="Backup gaps"/></div></section> : null}
      <div className="editorial-split">
        <section>
