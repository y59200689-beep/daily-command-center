...
export function InboxWorkbench({ rows, query, setQuery, onCapture, onEdit, onResolve, loading }: Props) {
  ...
  return <div className={`inbox-ui ${selected ? "inbox-ui--detail-open" : ""}`}>
    <section className="inbox-ui__list" aria-label="Captured items">
      ...
      <div className="inbox-ui__tools"><div><label><span className="sr-only">Category</span><select value={kind} onChange={event => setKind(event.target.value as typeof kind)}>{kinds.map(label => <option key={label} value={label}>{label} ({count(label)})</option>)}</select></label><label><span className="sr-only">Date filter</span><select value={timeFilter} onChange={event => setTimeFilter(event.target.value)}>{["All dates", "Today", "This week", "Later"].map(label => <option key={label}>{label}</option>)}</select></label></div><button className="inbox-ui__capture" onClick={onCapture}><Icons.Plus size={16}/> Capture</button></div>
