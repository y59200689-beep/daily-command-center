...
export async function POST(request:Request){try{const{ text }=inputSchema.parse(await request.json());const parsed=parseCapture(text);const{supabase,userId}=await requireUser();let record;
  ...
  else if(parsed.kind==="decision")record=await createRecord(supabase,userId,"decisions",{title:parsed.text.slice(0,120),decision:parsed.text,status:"proposed",decision_date:new Date().toISOString().slice(0,10)});
  ...
  else if(parsed.kind==="waiting")record=await createRecord(supabase,userId,"waiting",{title:parsed.text.slice(0,240),notes:parsed.text,expected_by:parsed.due??null,status:"waiting"});
  else if(parsed.kind==="knowledge")record=await createRecord(supabase,userId,"notes",{title:parsed.text.slice(0,120),content:parsed.text,category:"knowledge"});
  else record=await createRecord(supabase,userId,"inbox",{raw_text:parsed.text,detected_type:parsed.kind,confidence:parsed.confidence,status:"unprocessed"});
