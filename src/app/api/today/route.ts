...
export async function GET(){try{const{supabase,userId}=await requireUser();const profile=await supabase.from("profiles").select("display_name,timezone").eq("id",userId).maybeSingle();if(profile.error)throw profile.error;const timezone=profile.data?.timezone??"UTC";const today=dateInTimezone(new Date(),timezone);const {start,end}=dayBounds(today,timezone);
 ...
 const company=await supabase.from("companies").select("id").eq("user_id",userId).eq("active",true).maybeSingle();if(company.error&&!isMissingOptionalRelation(company.error))throw company.error;let founderSignals:{id:string;title:string;message:string;route:string;priority:number}[]=[];if(company.data){const[deployments,incidents]=await Promise.all([supabase.from("deployment_records").select("id,status,environment,commit_message").eq("user_id",userId).eq("company_id",company.data.id).eq("environment","production").eq("status","failed").limit(2),supabase.from("incidents").select("id,title,severity").eq("user_id",userId).eq("company_id",company.data.id).neq("status","resolved").limit(5)]);const founderFailure=[deployments,incidents].find(result=>result.error);if(founderFailure?.error&&!isMissingOptionalRelation(founderFailure.error))throw founderFailure.error;founderSignals=[...(incidents.data??[]).filter(item=>item.severity==="critical").map(item=>({id:`founder-incident:${item.id}`,title:"Critical production incident",message:item.title,route:"/founder",priority:100})),...(deployments.data??[]).map(item=>({id:`founder-deployment:${item.id}`,title:"Production deployment failed",message:item.commit_message??"Review the latest production deployment.",route:"/founder",priority:90}))].sort((a,b)=>b.priority-a.priority).slice(0,2)}
 let strategicSignals:{id:string;title:string;message:string;route:string;priority:number}[]=[];
  try{const [strategicCommitments,strategyGates]=await Promise.all([supabase.from("strategic_commitments").select("id,title,target_date,status").eq("user_id",userId).not("status","in","(completed,dropped)").order("target_date").limit(12),supabase.from("decision_gates").select("id,title,due_date").eq("user_id",userId).eq("status","open").order("due_date").limit(8)]);const strategyFailure=[strategicCommitments,strategyGates].find(result=>result.error);if(strategyFailure?.error&&!isMissingOptionalRelation(strategyFailure.error))throw strategyFailure.error;strategicSignals=[...(strategyGates.data??[]).filter(item=>item.due_date&&item.due_date<=today).map(item=>({id:"strategy-gate:"+item.id,title:"Decision due today",message:item.title+" blocks strategic progress.",route:"/control-tower",priority:95})),...(strategicCommitments.data??[]).filter(item=>item.status==="at_risk"&&item.target_date&&item.target_date<=new Date(Date.now()+86400000).toISOString().slice(0,10)).map(item=>({id:"strategy-commitment:"+item.id,title:"Strategic milestone needs attention",message:item.title+" is at risk near its target date.",route:"/control-tower",priority:80}))].sort((a,b)=>b.priority-a.priority).slice(0,2);}catch{/* strategic signals are optional; fail gracefully */}
 let knowledgeSignals:{id:string;title:string;message:string;route:string;priority:number}[]=[];
  try{const [topicsDue,staleSources,watchesDue]=await Promise.all([supabase.from("research_topics").select("id,title,next_review_at").eq("user_id",userId).neq("status","archived").not("next_review_at","is",null).lte("next_review_at",today).limit(3),supabase.from("knowledge_sources").select("id,title,freshness_expires_at").eq("user_id",userId).not("freshness_expires_at","is",null).lt("freshness_expires_at",today).limit(3),supabase.from("watch_entities").select("id,name,next_check_at").eq("user_id",userId).eq("status","active").not("next_check_at","is",null).lte("next_check_at",today).limit(3)]);const knowledgeFailure=[topicsDue,staleSources,watchesDue].find(result=>result.error);if(knowledgeFailure?.error&&!isMissingOptionalRelation(knowledgeFailure.error))throw knowledgeFailure.error;knowledgeSignals=[...(topicsDue.data??[]).map(item=>({id:`knowledge-topic:${item.id}`,title:"Topic review due",message:`${item.title} is scheduled for review.`,route:`/knowledge/topics/${item.id}`,priority:70})),...(staleSources.data??[]).map(item=>({id:`knowledge-source:${item.id}`,title:"Source freshness expired",message:`${item.title} needs re-verification.`,route:`/knowledge/sources/${item.id}`,priority:65})),...(watchesDue.data??[]).map(item=>({id:`knowledge-watch:${item.id}`,title:"Watch item check due",message:`${item.name} is due for scheduled check.`,route:`/knowledge/watch`,priority:60}))].sort((a,b)=>b.priority-a.priority).slice(0,2);}catch{/* knowledge signals are optional; fail gracefully */}
 const growthSignals=[...(opportunities.data??[]).filter(item=>(!item.next_action||!String(item.next_action).trim())&&Number(item.estimated_value??0)>=15000).map(item=>({id:`growth-opp:${item.id}`,title:"High-value deal has no next action",message:`${item.title} (${item.estimated_value} ${item.currency??"MAD"}) needs a scheduled sales action.`,route:`/pipeline?opportunity=${item.id}`,priority:Number(item.estimated_value??0)})),...(proposals.data??[]).filter(item=>item.valid_until&&item.valid_until<=new Date(Date.now()+4*86400000).toISOString().slice(0,10)).map(item=>({id:`growth-prop:${item.id}`,title:"Proposal follow-up required",message:`${item.title} expires soon (${item.valid_until}).`,route:`/proposals?proposal=${item.id}`,priority:Number(item.total??0)}))].sort((a,b)=>b.priority-a.priority).slice(0,2);
  let operationsSignals:{id:string;title:string;message:string;route:string;priority:number}[]=[];
  try{const [opRuns, opIncidents, opSops] = await Promise.all([supabase.from("process_runs").select("id,title,priority,status,due_at").eq("user_id", userId).in("status", ["failed", "blocked", "in_progress", "ready", "planned"]).limit(10),supabase.from("quality_incidents").select("id,title,severity,status").eq("user_id", userId).eq("severity", "critical").neq("status", "resolved").limit(3),supabase.from("operational_sops").select("id,title,criticality,next_review_at").eq("user_id", userId).eq("criticality", "critical").lte("next_review_at", today).limit(3)]);const opFailure=[opRuns,opIncidents,opSops].find(r=>r.error);if(opFailure?.error&&!isMissingOptionalRelation(opFailure.error))throw opFailure.error;operationsSignals=[...(opRuns.data??[]).filter(r=>r.status==="failed").map(r=>({id:`op-run-failed:${r.id}`,title:"Critical process failed",message:`${r.title} failed and needs operational review.`,route:`/operations/runs/${r.id}`,priority:100})),...(opIncidents.data??[]).map(i=>({id:`op-incident:${i.id}`,title:"Critical quality incident",message:`${i.title} is open.`,route:"/operations/quality",priority:95})),...(opRuns.data??[]).filter(r=>r.status==="blocked").map(r=>({id:`op-run-blocked:${r.id}`,title:"Process run blocked",message:`${r.title} is blocked.`,route:`/operations/runs/${r.id}`,priority:90})),...(opRuns.data??[]).filter(r=>["planned","ready","in_progress"].includes(r.status)&&r.due_at&&r.due_at.slice(0,10)<today).map(r=>({id:`op-run-overdue:${r.id}`,title:"Process run overdue",message:`${r.title} is past due date.`,route:`/operations/runs/${r.id}`,priority:85})),...(opSops.data??[]).map(s=>({id:`op-sop-review:${s.id}`,title:"Critical SOP review overdue",message:`${s.title} is due for scheduled review.`,route:`/operations/sops/${s.id}`,priority:80}))].sort((a,b)=>b.priority-a.priority).slice(0,2);}catch{/* operations signals are optional; fail gracefully */}
  let teamSignals:{id:string;title:string;message:string;route:string;priority:number}[]=[];
  try{const [tDels, tResps, tEscs] = await Promise.all([supabase.from("team_delegations").select("id,title,priority,status,due_at,blocked_reason").eq("user_id", userId).in("status", ["blocked", "assigned", "acknowledged", "in_progress"]).limit(10), supabase.from("team_responsibilities").select("id,name,criticality,primary_owner_id").eq("user_id", userId).eq("criticality", "critical").is("primary_owner_id", null).limit(3), supabase.from("team_escalations").select("id,reason,severity").eq("user_id", userId).eq("status", "open").eq("severity", "critical").limit(3)]);const teamFailure=[tDels,tResps,tEscs].find(r=>r.error);if(teamFailure?.error&&!isMissingOptionalRelation(teamFailure.error))throw teamFailure.error;teamSignals=[...(tEscs.data??[]).map(e=>({id:`team-esc:${e.id}`,title:"Critical team escalation",message:e.reason,route:"/team/risks",priority:100})),...(tDels.data??[]).filter(d=>d.status==="blocked"&&d.priority==="critical").map(d=>({id:`team-del-blocked:${d.id}`,title:"Critical delegation blocked",message:`${d.title} is blocked: ${d.blocked_reason||"needs unblocking"}`,route:"/team/delegations",priority:95})),...(tResps.data??[]).map(r=>({id:`team-resp-unowned:${r.id}`,title:"Critical responsibility has no owner",message:`Area "${r.name}" has no primary owner assigned.`,route:"/team/responsibilities",priority:90})),...(tDels.data??[]).filter(d=>d.priority==="critical"&&d.due_at&&d.due_at.slice(0,10)<today).map(d=>({id:`team-del-overdue:${d.id}`,title:"Critical delegation overdue",message:`${d.title} missed target due date.`,route:"/team/delegations",priority:85}))].sort((a,b)=>b.priority-a.priority).slice(0,2);}catch{/* team signals are optional; fail gracefully */}
  let customerSuccessSignals:{id:string;title:string;message:string;route:string;priority:number}[]=[];
  try{const [csRisks, csRenewals, csIssues, csCommitments] = await Promise.all([supabase.from("client_risks").select("id, client_id, description, severity").eq("user_id", userId).in("status", ["open", "mitigating"]).eq("severity", "critical").limit(3),supabase.from("client_renewals").select("id, client_id, renewal_date, preparation_state").eq("user_id", userId).in("status", ["upcoming", "preparing"]).lte("renewal_date", new Date(Date.now() + 30 * 86_400_000).toISOString().slice(0, 10)).limit(3),supabase.from("client_issues").select("id, client_id, title, severity").eq("user_id", userId).in("status", ["open", "investigating", "waiting_on_us"]).eq("severity", "critical").limit(3),supabase.from("client_commitments").select("id, client_id, statement, due_at").eq("user_id", userId).eq("status", "open").eq("direction", "we_owe_client").lt("due_at", today).limit(3)]);const csFailure=[csRisks,csRenewals,csIssues,csCommitments].find(r=>r.error);if(csFailure?.error&&!isMissingOptionalRelation(csFailure.error))throw csFailure.error;customerSuccessSignals=[...(csIssues.data??[]).map(i=>({id:`cs-issue:${i.id}`,title:"Critical client issue open",message:i.title,route:`/success/clients/${i.client_id}`,priority:100})),...(csRisks.data??[]).map(r=>({id:`cs-risk:${r.id}`,title:"Critical client risk",message:r.description,route:`/success/clients/${r.client_id}`,priority:95})),...(csCommitments.data??[]).map(c=>({id:`cs-commit:${c.id}`,title:"Client promise overdue",message:c.statement,route:`/success/clients/${c.client_id}`,priority:90})),...(csRenewals.data??[]).filter(ren=>ren.preparation_state==="not_started").map(ren=>({id:`cs-renewal:${ren.id}`,title:"Client renewal approaching",message:`Renewal due ${ren.renewal_date} has no prep started.`,route:`/success/clients/${ren.client_id}`,priority:85}))].sort((a,b)=>b.priority-a.priority).slice(0,2);}catch{/* customer success signals are optional; fail gracefully */}
...
function isMissingOptionalRelation(error:unknown){return Boolean(error&&typeof error==="object"&&"code" in error&&((error as {code?:unknown}).code==="PGRST205"||(error as {code?:unknown}).code==="42P01"))}
