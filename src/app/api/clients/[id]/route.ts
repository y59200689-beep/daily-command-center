...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const queries = await Promise.all([
      ...
      supabase.from("email_threads").select("*").eq("user_id",userId).eq("linked_client_id",id).order("last_message_at",{ascending:false}).limit(5),
      supabase.from("activity_log").select("*").eq("user_id",userId).eq("entity_type","client").eq("entity_id",id).order("created_at",{ascending:false}).limit(30),
    ]);
    ...
    const meetingActivity = activity.map((item) => ({ ...item, title: item.action === "meeting_outcome_captured" ? `Meeting completed · ${String((item.metadata as Record<string, unknown> | null)?.meeting_title ?? "Outcome captured")}` : item.action, status: String((item.metadata as Record<string, unknown> | null)?.summary ?? item.created_at) }));
    const timeline = [...meetingActivity, ...followups, ...waiting, ...notes, ...content, ...enriched, ...payments].sort((a, b) => String(b.updated_at ?? b.created_at ?? b.payment_date ?? "").localeCompare(String(a.updated_at ?? a.created_at ?? a.payment_date ?? ""))).slice(0, 50);

    return NextResponse.json({
      client: client.data,
      related: { projects, tasks, followups, waiting, notes, content, invoices: enriched, payments, files, threads, timeline },
