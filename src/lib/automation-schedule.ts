...
export function nextAutomationRun(type: string, enabled: boolean, schedule: Record<string, unknown>, timezone = "Africa/Casablanca", now = new Date()) {
  ...
  const days = Array.isArray(schedule.days) ? schedule.days.map(Number) : ["weekly_review", "weekly_knowledge_review", "weekly_growth_review"].includes(type) ? [Number(schedule.day ?? 0)] : [0, 1, 2, 3, 4, 5, 6];
...
export function automationFields(type: string) {
  if (["morning_brief", "evening_review", "daily_sales_attention"].includes(type)) return ["time", "days"];
  if (["weekly_review", "weekly_planning_reminder", "weekly_knowledge_review", "weekly_growth_review"].includes(type)) return ["time", "day"];
