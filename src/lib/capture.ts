export type CaptureKind = "task"|"note"|"idea"|"decision"|"followup"|"inbox"|"issue"|"risk"|"waiting"|"commitment"|"contact"|"experiment"|"knowledge"|"forecast"|"content"|"asset";
...
export function parseCapture(input:string, now=new Date()):ParsedCapture{
  ...
  const prefixes:[RegExp,CaptureKind,number][]=[[/^issue\s+/i,"issue",.98],[/^risk\s+/i,"risk",.98],[/^waiting(?: for)?\s+/i,"waiting",.98],[/^commitment\s+/i,"commitment",.98],[/^contact\s+/i,"contact",.98],[/^experiment\s+/i,"experiment",.98],[/^forecast\s+/i,"forecast",.98],[/^content\s+/i,"content",.98],[/^asset\s+/i,"asset",.98],[/^knowledge\s+/i,"knowledge",.98],[/^task\s+/i,"task",.98],[/^note\s+/i,"note",.98],[/^idea\s+/i,"idea",.98],[/^decision\s+/i,"decision",.98],[/^follow[ -]?up(?:\s+with)?\s+/i,"followup",.96]];
