import { z } from "zod";
import {financialAssistantTools,financialReadNames,financialWrites,executeFinancialTool} from './financial-ai';
import {executiveAssistantTools,executiveReadNames,executiveWrites,executeExecutiveTool} from './executive-ai';
import {chiefOfStaffAssistantTools,chiefOfStaffReadNames,chiefOfStaffWrites,executeChiefOfStaffTool} from './chief-of-staff-ai';
import {learningAssistantTools,learningReadNames,learningWrites,executeLearningTool} from './learning-ai';
...
export const assistantTools=[
 ...
 tool("get_fitness_progress","Get the current owned fitness progress for Life.",{}),tool("add_visa_document_requirement","Add a user-defined visa checklist requirement after explicit confirmation.",{visa_application_id:string,title:string,confirmed:{type:"boolean"}},["visa_application_id","title","confirmed"]),
  ...
  // V14 Commerce tools
  tool("get_commerce_overview","Get commerce overview: stock risk distribution, reorder counts, next commerce move, and currency-separated inventory metrics.",{}),tool("get_inventory_intelligence","Get product inventory intelligence with coverage days, velocity, stockout risk, and reorder state.",{}),tool("get_replenishment_queue","Get prioritized replenishment queue of products needing reorder now or soon.",{}),tool("get_slow_stock","Get slow-moving and dead-stock candidates with dormant capital estimates.",{}),tool("get_product_intelligence","Get product performance: velocity, revenue, margin, and momentum per product.",{}),tool("get_commerce_suppliers","Get supplier reliability assessments: lead times, fill rates, late rate, and reliability classification.",{}),tool("get_purchase_review","Get purchasing review: supplier orders that are late, partial, or need attention.",{}),tool("get_commerce_risks","Get all active commerce risks: stockouts, overstock, supplier delays, cost gaps, and PO issues.",{}),tool("get_commerce_review","Get commerce review summary with weekly and monthly metrics.",{period:{type:"string",enum:["weekly","monthly"]}}),tool("get_inventory_discrepancies","Get open inventory discrepancies under investigation or pending resolution.",{}),tool("get_inventory_audits","Get inventory cycle count audits and their completion state.",{}),tool("get_inventory_movements","Get stock movement audit trail: receipts, sales, adjustments, and corrections.",{}),tool("get_inventory_scenarios","Get saved what-if inventory scenarios.",{}),tool("get_commerce_brands","Get brand-level inventory and revenue aggregations.",{}),tool("get_commerce_categories","Get category-level product performance and risk distribution.",{}),tool("get_next_commerce_move","Get the single highest-priority commerce intervention requiring owner attention.",{}),
  tool("record_inventory_discrepancy","Record an inventory discrepancy for investigation after explicit confirmation.",{product_id:string,discrepancy_type:string,expected_quantity:{type:"number"},observed_quantity:{type:"number"},notes:nullableString,confirmed:{type:"boolean"}},["product_id","discrepancy_type","expected_quantity","observed_quantity","confirmed"]),tool("resolve_inventory_discrepancy","Mark an inventory discrepancy as resolved after explicit confirmation.",{id:string,resolution_notes:string,confirmed:{type:"boolean"}},["id","resolution_notes","confirmed"]),tool("create_inventory_adjustment","Record an audited stock adjustment with a mandatory reason after explicit confirmation.",{product_id:string,quantity_delta:{type:"number"},reason:string,confirmed:{type:"boolean"}},["product_id","quantity_delta","reason","confirmed"]),
  // V16 Executive Intelligence tools
  ...executiveAssistantTools,
  // V17 Chief of Staff tools
  ...chiefOfStaffAssistantTools,
  // V18 Operating Memory & Learning tools
...
export async function executeAssistantTool(ctx:Ctx,name:string,raw:unknown){
 if((financialReadNames as readonly string[]).includes(name)||financialWrites[name])return executeFinancialTool(ctx.client,ctx.userId,name,raw);
 if((executiveReadNames as readonly string[]).includes(name)||Boolean(executiveWrites[name]))return executeExecutiveTool(ctx.client,ctx.userId,name,(raw ?? {}) as Record<string, unknown>);
 if((chiefOfStaffReadNames as readonly string[]).includes(name)||Boolean(chiefOfStaffWrites[name]))return executeChiefOfStaffTool(ctx.client,ctx.userId,name,(raw ?? {}) as Record<string, unknown>);
 if((learningReadNames as readonly string[]).includes(name)||Boolean(learningWrites[name]))return executeLearningTool(ctx.client,ctx.userId,name,(raw ?? {}) as Record<string, unknown>);
 ...
 const commerceReadTools=new Set(["get_commerce_overview","get_inventory_intelligence","get_replenishment_queue","get_slow_stock","get_product_intelligence","get_commerce_suppliers","get_purchase_review","get_commerce_risks","get_commerce_review","get_inventory_discrepancies","get_inventory_audits","get_inventory_movements","get_inventory_scenarios","get_commerce_brands","get_commerce_categories","get_next_commerce_move"]);
  if(commerceReadTools.has(name)){const {loadFullCommerceState}=await import("@/lib/commerce-server");const company=await ctx.client.from("companies").select("id").eq("user_id",ctx.userId).eq("active",true).maybeSingle();if(company.error||!company.data)return{state:"not_configured",message:"Set up a company before using commerce tools."};const state=await loadFullCommerceState(ctx.client,ctx.userId,company.data.id);if(name==="get_inventory_intelligence")return state.products;if(name==="get_replenishment_queue")return state.products.filter((p:Record<string,unknown>)=>p.reorder&&["reorder_now","review_soon"].includes((p.reorder as Record<string,unknown>).state as string));if(name==="get_slow_stock")return state.slowStock;if(name==="get_product_intelligence")return state.products;if(name==="get_commerce_suppliers")return state.suppliers;if(name==="get_purchase_review")return state.supplierOrders.filter((o:Record<string,unknown>)=>o.health&&["late","partial","pending"].includes((o.health as Record<string,unknown>).state as string));if(name==="get_commerce_risks")return state.activeRisks;if(name==="get_inventory_discrepancies")return state.discrepancies.filter((d:Record<string,unknown>)=>["investigating","verified"].includes(String(d.status)));if(name==="get_inventory_audits")return state.audits;if(name==="get_inventory_movements")return state.adjustments;if(name==="get_inventory_scenarios"){const{data}=await ctx.client.from("saved_inventory_scenarios").select("*").eq("user_id",ctx.userId).eq("company_id",company.data.id).limit(50);return data??[];}if(name==="get_commerce_brands"){const bMap:Record<string,{name:string;productCount:number;availableStockUnits:number;soldUnits30d:number}>={};for(const p of state.products){const b=p.brand?.trim()||"Unbranded";if(!bMap[b])bMap[b]={name:b,productCount:0,availableStockUnits:0,soldUnits30d:0};bMap[b].productCount++;bMap[b].availableStockUnits+=p.availableStock;bMap[b].soldUnits30d+=p.totalSold30d;}return Object.values(bMap);}if(name==="get_commerce_categories"){const cMap:Record<string,{name:string;productCount:number;availableStockUnits:number;soldUnits30d:number}>={};for(const p of state.products){const c=p.category?.trim()||"Uncategorized";if(!cMap[c])cMap[c]={name:c,productCount:0,availableStockUnits:0,soldUnits30d:0};cMap[c].productCount++;cMap[c].availableStockUnits+=p.availableStock;cMap[c].soldUnits30d+=p.totalSold30d;}return Object.values(cMap);}if(name==="get_next_commerce_move")return state.nextMove;return{overview:{productsCount:state.products.length,suppliersCount:state.suppliers.length,reorderNowCount:state.products.filter((p:Record<string,unknown>)=>p.reorder&&(p.reorder as Record<string,unknown>).state==="reorder_now").length,slowStockCount:state.slowStock.length,lateOrdersCount:state.supplierOrders.filter((o:Record<string,unknown>)=>o.health&&(o.health as Record<string,unknown>).state==="late").length,currencyBuckets:state.currencyBuckets,missingCostCount:state.missingCostCount},nextMove:state.nextMove,topRisks:state.activeRisks.slice(0,5)};}
 if(["record_inventory_discrepancy","resolve_inventory_discrepancy","create_inventory_adjustment"].includes(name)){const confirmed=z.object({confirmed:z.boolean()}).passthrough().parse(raw).confirmed;const blocked=writeGuard(confirmed);if(blocked)return blocked;if(name==="record_inventory_discrepancy"){const input=z.object({product_id:z.uuid(),discrepancy_type:z.string().min(1),expected_quantity:z.number(),observed_quantity:z.number(),notes:z.string().nullable().optional(),confirmed:z.boolean()}).parse(raw);const{data,error}=await ctx.client.from("inventory_discrepancies").insert({user_id:ctx.userId,product_id:input.product_id,discrepancy_type:input.discrepancy_type,expected_quantity:input.expected_quantity,observed_quantity:input.observed_quantity,notes:input.notes??null,status:"investigating"}).select("*").single();if(error)throw error;return data}if(name==="resolve_inventory_discrepancy"){const input=z.object({id:z.uuid(),resolution_notes:z.string().min(1),confirmed:z.boolean()}).parse(raw);const{data,error}=await ctx.client.from("inventory_discrepancies").update({status:"resolved",resolution_notes:input.resolution_notes,resolved_at:new Date().toISOString()}).eq("id",input.id).eq("user_id",ctx.userId).select("*").single();if(error)throw error;return data}const input=z.object({product_id:z.uuid(),quantity_delta:z.number(),reason:z.string().min(1),confirmed:z.boolean()}).parse(raw);const{data,error}=await ctx.client.from("inventory_adjustments").insert({user_id:ctx.userId,product_id:input.product_id,quantity_delta:input.quantity_delta,reason:input.reason,adjusted_at:new Date().toISOString()}).select("*").single();if(error)throw error;return data}
