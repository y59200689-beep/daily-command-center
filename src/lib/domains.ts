...
const schemas = {
  ...
  notes: z.object({ title: z.string().trim().min(1).max(240), content: z.string().max(20000).default(""), category: z.string().trim().max(80).default("note"), project_id: optionalUuid, client_id: optionalUuid }),
  ...
  content: z.object({ title: z.string().trim().min(1).max(240), client_id: optionalUuid, project_id: optionalUuid, campaign_id: optionalUuid, prompt_id: optionalUuid, platform: z.enum(["Instagram","Facebook","TikTok","LinkedIn","YouTube","Website","Email","Other"]).optional().nullable(), format: optionalText, status: z.enum(["idea","brief","copy","designing","review","approved","scheduled","published","archived"]).default("idea"), priority: z.enum(["none","low","medium","high","urgent"]).default("none"), hook: optionalText, caption: optionalText, creative_brief: optionalText, creative_direction: optionalText, cta: optionalText, publish_date: dateTime, scheduled_at: dateTime, published_at: dateTime, due_date: date, approval_status: z.enum(["not_required","pending","changes_requested","approved"]).default("not_required"), approval_notes: optionalText, approved_by_name: z.string().trim().max(240).optional().nullable(), performance_notes: optionalText }),
  fitness: z.object({ activity_type: z.enum(["Running","Gym","Walking","Swimming","Hiking","Cycling","Other"]), date: z.iso.date(), duration_minutes: z.coerce.number().int().positive().optional().nullable(), distance_km: money.optional().nullable(), calories: z.coerce.number().int().nonnegative().optional().nullable(), source: z.string().trim().max(60).default("manual"), external_id: optionalText, notes: optionalText, effort: z.enum(["easy","moderate","hard"]).optional().nullable() }),
