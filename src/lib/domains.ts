...
const schemas = {
  tasks: z.object({ title: z.string().trim().min(1).max(240), description: optionalText, status: z.enum(["inbox","planned","in_progress","waiting","blocked","completed","cancelled"]).default("inbox"), priority: z.enum(["none","low","medium","high","urgent"]).default("none"), project_id: optionalUuid, client_id: optionalUuid, goal_id: optionalUuid, scope_item_id: optionalUuid, trip_id: optionalUuid, scope_status: z.preprocess(normalizeBlankToNull, z.enum(["in_scope","out_of_scope","unclear"]).nullable().optional()), due_date: date, estimated_minutes: optionalInteger(1, 1440), completed_at: dateTime, daily_position: optionalInteger(1, 3), work_classification: z.enum(["founder_only","delegate","automate_candidate","standard"]).optional(), recurrence_frequency: z.preprocess(normalizeBlankToNull, z.enum(["daily","weekly","monthly","mon","tue","wed","thu","fri","sat","sun"]).nullable().optional()), target_count: optionalNullableNumber(z.number().finite().positive()), current_count: optionalNullableNumber(z.number().finite().nonnegative()), daily_target: optionalNullableNumber(z.number().finite().positive()) }),
  ...
  waiting: z.object({ followup_date: date, importance: z.enum(["low","medium","high","critical"]).default("medium"), direction: z.enum(["owed_to_me","owed_by_me"]).default("owed_to_me"), impact: optionalText, task_id: optionalUuid, company_id: optionalUuid, owner_label: optionalText, blocking_revenue: z.boolean().default(false), title: z.string().trim().min(1).max(240), client_id: optionalUuid, project_id: optionalUuid, contact: optionalText, expected_by: dateTime, status: z.string().trim().max(40).default("waiting"), notes: optionalText }),
  notes: z.object({ title: z.string().trim().min(1).max(240), content: defaultedNoteContent, category: defaultedText("note", 80), project_id: optionalUuid, client_id: optionalUuid, trip_id: optionalUuid }),
  goals: z.object({ title: z.string().trim().min(1).max(240), description: optionalText, period: z.enum(["year","quarter","month","week"]).default("quarter"), target_date: date, status: z.string().trim().max(40).default("active"), progress: z.coerce.number().min(0).max(100).default(0) }),
  ...
  decisions: z.object({ options_considered: optionalText, selected_option: optionalText, assumptions: optionalText, expected_outcome: optionalText, expected_metric: optionalText, actual_outcome: optionalText, variance: optionalText, lesson_learned: optionalText, owner_label: optionalText, company_id: optionalUuid, deadline: date, currency, reversibility: z.enum(["reversible","costly","irreversible"]).nullable().optional(), financial_exposure: optionalNullableNumber(z.number().finite().nonnegative()), title: z.string().trim().min(1).max(240), decision: z.string().trim().min(1).max(10000), reasoning: optionalText, project_id: optionalUuid, client_id: optionalUuid, impact: z.enum(["low","medium","high","critical"]).default("medium"), confidence: z.enum(["low","medium","high"]).default("medium"), status: z.enum(["active","review_due","superseded","reversed","archived","proposed","under_review","decided","implemented","validated"]).default("active"), decision_date: date, review_date: date, superseded_by_decision_id: optionalUuid }),
  ...
  "notification-preferences": z.object({ category:z.enum(["tasks","calendar","clients","finance","content","decisions","fitness","integrations","automations"]),enabled:z.boolean().default(true),minimum_severity:z.enum(["low","medium","high","critical"]).default("medium") }),
