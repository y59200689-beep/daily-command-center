...
import { crossDomainSynthesis, forecastCalibration, tier3MeaningfulFacts } from "./tier3";
...
export async function loadTier1Briefing(client: SupabaseClient, userId: string, today: string, window: number) {
  ...
  const tables = {
    wealth: "personal_balance_entries", wealthHistory: "personal_balance_history", documents: "personal_documents", trips: "trips",
    tasks: "tasks", projects: "projects", decisions: "decisions", issues: "quality_incidents", risks: "operating_risks",
    waiting: "waiting_items", commitments: "operating_commitments", relationships: "operating_relationships",
    dependencies: "operational_dependencies", systems: "operational_systems", access: "system_access_records",
    healthChecks: "system_health_checks", delegations: "team_delegations", responsibilities: "team_responsibilities",
    milestones: "strategic_milestones", strategicCommitments: "strategic_commitments", strategyLinks: "founder_strategy_links",
    lessons: "operating_lessons", forecasts: "founder_forecasts", evaluations: "forecast_evaluations", invoices: "invoices",
    incidents: "incidents", customerOutcomes: "client_outcomes", sessions: "focus_sessions",
    experiments: "growth_experiments", assets: "asset_metadata", content: "content_items", dailyStates: "founder_daily_states"
  };
  ...
  await Promise.all(Object.entries(tables).map(async ([key, table]) => {
    ...
    if (["tasks", "projects", "decisions", "waiting_items", "invoices", "content_items"].includes(table)) query = query.is("deleted_at", null);
    ...
    const result = await query.order(["sessions", "forecasts", "evaluations"].includes(key) ? (key === "sessions" ? "started_at" : "created_at") : ["strategyLinks", "lessons"].includes(key) ? "created_at" : key === "dailyStates" ? "date" : "updated_at", { ascending: false }).limit(501);
  ...
  facts.push(...tier3MeaningfulFacts(data, today));
  ...
  const syntheses = crossDomainSynthesis(data, today);
  const calibration = forecastCalibration(data.forecasts ?? []);
  const weekly = {
    ...weeklyBriefing(data, today),
    "Business state changes": (changes ?? []).filter(r => r.domain === "Business").map(r => ({ id: r.id, title: r.title, route: r.route, why: `${r.before} → ${r.state}` })),
    "Cross-Domain Synthesis": syntheses.map(s => ({ id: s.id, title: s.title, route: s.route, why: `${s.observation} · ${s.whyItMatters}` })),
    "Experiments & Learning": (data.experiments ?? []).filter(e => e.outcome_result || e.status === "completed").slice(0, 8).map(r => ({ id: r.id, title: String(r.name), route: `/experiments?record=${r.id}`, why: `Outcome: ${r.outcome_result ?? "pending"} · ${r.target_metric ?? "metric unassigned"}` })),
    "Forecast Calibration": (data.forecasts ?? []).filter(f => f.resolution && f.resolution !== "unresolved").slice(0, 8).map(r => ({ id: r.id, title: String(r.prediction), route: `/forecasts?record=${r.id}`, why: `Resolution: ${r.resolution} · Confidence: ${r.confidence}` })),
    "Asset Intelligence": (data.assets ?? []).filter(a => a.value_category === "core" || a.reuse_potential === "high").slice(0, 8).map(r => ({ id: r.id, title: String(r.name), route: `/assets?record=${r.id}`, why: `${r.value_category ?? "core"} · reuse ${r.reuse_potential ?? "medium"}` }))
  };
  return { today, window, pulse, finance: financialSummary(financial), attention, coverage, state, waiting: waitingPerspectives(data, today), bottlenecks: founderBottlenecks(data, today), momentum: data.projects.filter(r => ["active", "planning"].includes(String(r.status))).map(r => projectMomentum(r, data, today)), alignment: strategicAlignment(data), weekly, changes, syntheses, calibration, comparisonDate: previous.data?.snapshot_date ?? null, facts };
}
